...
def run_step1_8(context: Context, user_input: str, state: dict | None = None) -> tuple[Context, dict]:
    """Step 1.8: Response Obligation observation."""
    from graph.nodes import step1_8_response_obligation_observation
    next_state = {**(state or {}), "context": context, "user_input": user_input}
    result = step1_8_response_obligation_observation(next_state)
    return result.get("context", context), result
...
def execute_streaming_response(
    ...
    if state.get("skip_to_end"):
        ...
        observe_final_output(state, context, final_output, fallback_used=False)
    ...
    started_at = time.perf_counter()
    context, state = run_step1_8(context, user_input, state)
    _record_step_timing(step_timings, "step1_8_obligation", started_at)
    ...
    context.output = final_output
    from graph.nodes.response_obligation_observer import observe_final_output
    observe_final_output(state, context, final_output, fallback_used=getattr(context, "runtime_trace", {}).get("output_path") == "fallback")
    from graph.nodes.answer_first_takeover import apply_answer_first_takeover
    final_output = apply_answer_first_takeover(state, context, final_output)
    context.output = final_output
    observe_final_output(state, context, final_output, fallback_used=getattr(context, "runtime_trace", {}).get("output_path") == "fallback")
