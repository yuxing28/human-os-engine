...
from graph.nodes.response_obligation_observer import observe_final_output
...
def _polish_light_casual_output(context, user_input: str, final_output: str) -> tuple[str, bool, str]:
    """只在轻量轮次里，把明显像占位符的短答换成自然接话。"""
    text = (final_output or "").strip()
    if not text:
        return text, False, "empty"
    if _contains_crisis_intent(user_input):
        return text, False, "skip_crisis"

    trace = getattr(context, "runtime_trace", None)
    obligation = trace.get("response_obligation", {}) if isinstance(trace, dict) else {}
    if isinstance(obligation, dict):
        if obligation.get("identity_answer_required") or obligation.get("expected_deliverable") == "identity_answer":
            return text, False, "skip_identity"
        if obligation.get("repair_required"):
            return text, False, "skip_repair"

    scene = context.primary_scene or (context.scene_config.scene_id if context.scene_config else "")
    turn_load_level = getattr(context, "turn_load_level", "")
    if turn_load_level and turn_load_level != "light" and not getattr(context, "short_utterance", False):
        return text, False, "skip_not_light"
    is_light = (
        getattr(context, "short_utterance", False)
        or turn_load_level == "light"
        or _turn_behavior_profile(context, user_input, scene)["level"] == "light"
    )
    if not is_light:
        return text, False, "skip_not_light"

    reason = str(getattr(context, "short_utterance_reason", "") or "")
    compact = re.sub(r"\s+", "", text)
    mechanical = compact in {"嗯。", "嗯", "没问题。", "没问题", "...", "……"}
    step0_default = (
        compact == "嗯，我在。你要是想继续，我们接着说；如果想先停一下，也可以。"
        or compact == "我在。你可以再多说一点，我帮你接住。"
    )
    if not mechanical and not step0_default:
        return text, False, "already_natural"

    if reason == "quick_ack":
        return "好，我跟着。", True, "quick_ack"
    if reason == "ultra_short":
        return "慢慢说，我听着。", True, "ultra_short"
    if reason == "repeat":
        return "我看到了，我们接着这点走。", True, "repeat"
    if reason == "meta_identity":
        return text, False, "skip_identity_reason"
    if compact in {"...", "……"}:
        return "我先停一下。", True, "pause"
    return "可以，我压短说。", True, "light_placeholder"


def _continuation_context_hint(context) -> tuple[bool, str, str]:
    trace = getattr(context, "runtime_trace", None)
    trace = trace if isinstance(trace, dict) else {}
    route_state = getattr(context, "route_state", None) or trace.get("route_state", {})
    route_state = route_state if isinstance(route_state, dict) else {}
    context_brief = getattr(context, "context_brief", None) or trace.get("context_brief", {})
    context_brief = context_brief if isinstance(context_brief, dict) else {}
    obligation = trace.get("response_obligation", {})
    obligation = obligation if isinstance(obligation, dict) else {}

    phase = str(route_state.get("conversation_phase", "") or "")
    source = str(obligation.get("source", "") or "")
    next_pickup = str(context_brief.get("next_pickup", "") or "").strip()
    current_task = str(obligation.get("current_task", "") or "").strip()

    if phase in {"followup", "continuation"}:
        return True, "route_state", next_pickup or current_task
    if source == "inherited_context":
        return True, "response_obligation", next_pickup or current_task
    if next_pickup:
        return True, "next_pickup", next_pickup
    return False, "no_continuation_context", ""


def _polish_continuation_output(context, user_input: str, final_output: str) -> tuple[str, bool, str]:
    """只在已有继承上下文时，把空泛续接改成更像接着说的一句话。"""
    text = (final_output or "").strip()
    if not text:
        return text, False, "empty"

    trace = getattr(context, "runtime_trace", None)
    route_state = getattr(context, "route_state", None) or (trace.get("route_state", {}) if isinstance(trace, dict) else {})
    route_state = route_state if isinstance(route_state, dict) else {}
    route_risk = str(route_state.get("risk_level", "") or "")
    route_phase = str(route_state.get("conversation_phase", "") or "")
    if _contains_crisis_intent(user_input) or route_risk == "crisis" or route_phase.startswith("crisis"):
        return text, False, "skip_crisis"
    ...
    has_context, source, hint = _continuation_context_hint(context)
    if not has_context:
        return text, False, "skip_no_context"

    compact = re.sub(r"\s+", "", text)
    question_only = compact.endswith(("?", "？")) and len(compact) <= 28
    mechanical = compact in {"嗯。", "嗯", "没问题。", "没问题", "...", "……", "好。", "好的。"}
    generic_continue = compact in {
        "你想继续哪部分？",
        "你想往下说哪一块？",
        "可以继续说说。",
        "我们继续。",
    ...
    if not (question_only or mechanical or generic_continue):
        return text, False, "already_natural"

    if hint:
        clean_hint = re.sub(r"\s+", " ", hint).strip("。；; ")
        if len(clean_hint) > 28:
            clean_hint = clean_hint[:28].rstrip("，,、 ") + "..."
        return f"好，接着刚才那个点往下说。先看：{clean_hint}。", True, source
    return "好，接着刚才那个点往下说。", True, source


def _apply_continuation_polish(state, context, user_input: str, final_output: str) -> str:
    polished, used, reason = _polish_continuation_output(context, user_input, final_output)
    trace = _ensure_runtime_trace(state, context)
    trace["continuation_polish_used"] = bool(used)
    trace["continuation_polish_reason"] = reason
    if used:
        trace["continuation_polish_source"] = "final_expression_layer"
    _set_state_value(state, context, "runtime_trace", trace)
    return polished


def _first_sentence(text: str) -> str:
    match = re.match(r"^(.+?[。！？!?])", (text or "").strip())
    return match.group(1) if match else (text or "").strip()


def _warm_emotional_support_first_sentence(context, user_input: str, final_output: str) -> tuple[str, bool, str]:
    """只在非危机场景的 emotional_support 里，让第一句先接住人。"""
    text = (final_output or "").strip()
    if not text:
        return text, False, "empty"
    ...
    route_state = route_state if isinstance(route_state, dict) else {}
    if (
        _contains_crisis_intent(user_input)
        or str(route_state.get("risk_level", "") or "") == "crisis"
        or str(route_state.get("conversation_phase", "") or "").startswith("crisis")
    ):
        return text, False, "skip_crisis"

    obligation = trace.get("response_obligation", {}) if isinstance(trace, dict) else {}
    obligation = obligation if isinstance(obligation, dict) else {}
    if obligation.get("expected_deliverable") != "emotional_support":
        return text, False, "skip_not_emotional_support"
    if obligation.get("identity_answer_required") or obligation.get("expected_deliverable") == "identity_answer":
        return text, False, "skip_identity"
    if obligation.get("repair_required"):
        return text, False, "skip_repair"
    ...
    warm_anchors = ["刺", "慌", "堵", "委屈", "难受", "发紧", "不舒服", "被推开", "撑着", "累"]
    advice_first = compact_first.startswith(("你可以", "建议", "先", "直接结论", "可以先", "这件事可以"))
    generic_first = compact_first in {"我理解你的感受。", "这确实让人难受。", "你现在难受是正常的。"}
    if not advice_first and not generic_first and any(anchor in first for anchor in warm_anchors):
        return text, False, "already_warm"

    variants = [
        "这一下确实会让人心里发紧，先别急着逼自己马上处理好。",
        "你现在会慌不奇怪，这种话听起来就像突然被推开了。",
        "这事不只是讲道理的问题，难受和委屈先冒出来很正常。",
        "你先别急着责怪自己，这一刻心里堵住是很真实的反应。",
    ]
    seed = str(getattr(context, "session_id", "") or user_input or text)
    replacement = variants[int(hashlib.sha1(seed.encode("utf-8")).hexdigest(), 16) % len(variants)]
    rest = text[len(first):].lstrip() if first and text.startswith(first) else text
    if rest:
        return f"{replacement}{rest}", True, "first_sentence_warmed"
    return f"{replacement}先做一个小动作：把争辩停一下，等情绪落一点再确认真正的问题。", True, "first_sentence_warmed"


def _apply_emotional_support_warmth_polish(state, context, user_input: str, final_output: str) -> str:
    polished, used, reason = _warm_emotional_support_first_sentence(context, user_input, final_output)
    trace = _ensure_runtime_trace(state, context)
    trace["emotional_support_warmth_polish_used"] = bool(used)
    trace["emotional_support_warmth_polish_reason"] = reason
    if used:
        trace["emotional_support_warmth_polish_source"] = "final_expression_layer"
    _set_state_value(state, context, "runtime_trace", trace)
    return polished


def _apply_light_casual_polish(state, context, user_input: str, final_output: str) -> str:
    polished, used, reason = _polish_light_casual_output(context, user_input, final_output)
    trace = _ensure_runtime_trace(state, context)
    trace["light_casual_polish_used"] = bool(used)
    trace["light_casual_polish_reason"] = reason
    if used:
        trace["light_casual_polish_source"] = "final_expression_layer"
    _set_state_value(state, context, "runtime_trace", trace)
    return polished
...
def _run_step8_minimal_execution(state: GraphState, *, step8_mode: str) -> GraphState:
    ...
    final_output = _apply_emotional_support_warmth_polish(state, context, user_input, final_output)
    final_output = _apply_continuation_polish(state, context, user_input, final_output)
    final_output = _apply_light_casual_polish(state, context, user_input, final_output)
    ...
    final_output = apply_identity_truth_takeover(state, context, final_output)
    observe_final_output(state, context, final_output, fallback_used=fallback_focus_applied)
    final_output = apply_answer_first_takeover(state, context, final_output)
    observe_final_output(state, context, final_output, fallback_used=fallback_focus_applied)
    final_output = apply_internal_strategy_leak_takeover(state, context, final_output)
    observe_final_output(state, context, final_output, fallback_used=fallback_focus_applied)
    final_output = apply_repair_contract_takeover(state, context, final_output)
    observe_final_output(state, context, final_output, fallback_used=fallback_focus_applied)
    final_output = apply_fallback_contract_takeover(state, context, final_output)
    observe_final_output(state, context, final_output, fallback_used=fallback_focus_applied)
...
def step8_execution(state: GraphState) -> GraphState:
    ...
    if state.get("skip_to_end", False) and context.output:
        ...
        final_output = apply_identity_truth_takeover(state, context, final_output)
        observe_final_output(state, context, final_output, fallback_used=continuity_focus_used)
        final_output = apply_answer_first_takeover(state, context, final_output)
        observe_final_output(state, context, final_output, fallback_used=continuity_focus_used)
        final_output = apply_internal_strategy_leak_takeover(state, context, final_output)
        observe_final_output(state, context, final_output, fallback_used=continuity_focus_used)
    ...
    observe_final_output(
        ...
        context,
        final_output,
        fallback_used=runtime_trace.get("output_path") == "fallback",
    )
    final_output = apply_answer_first_takeover(state, context, final_output)
    observe_final_output(
        state,
        context,
        final_output,
        fallback_used=bool(fallback_focus_applied or runtime_trace.get("output_path") == "fallback"),
    )
    final_output = apply_internal_strategy_leak_takeover(state, context, final_output)
    observe_final_output(
        state,
