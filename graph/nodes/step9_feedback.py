...
from graph.nodes.response_obligation_observer import attach_observation_to_latest_system_history
...
def _refresh_memory_trace_stats(context) -> None:
    ...
    try:
        ...
        trace["session_note_duplicate_score"] = float(note_stats.get("duplicate_score", 0.0) or 0.0)
        trace["session_note_repeated_entities"] = list(note_stats.get("repeated_entities", []) or [])
        trace["session_note_repeated_intents"] = list(note_stats.get("repeated_intents", []) or [])
        trace["session_note_compaction_needed"] = bool(note_stats.get("compaction_needed", False))
        trace["session_note_compaction_reason"] = list(note_stats.get("compaction_reason", []) or [])
        trace["world_state_duplicate_score"] = float(note_stats.get("world_state_duplicate_score", 0.0) or 0.0)
        trace["world_state_stale_risk"] = bool(note_stats.get("world_state_stale_risk", False))
        trace["world_state_conflict_detected"] = bool(note_stats.get("world_state_conflict_detected", False))
        trace["world_state_latest_update_source"] = str(note_stats.get("world_state_latest_update_source", "") or "")
        trace["next_pickup_duplicate_with_session_note"] = bool(note_stats.get("next_pickup_duplicate_with_session_note", False))
        trace["next_pickup_duplicate_with_world_state"] = bool(note_stats.get("next_pickup_duplicate_with_world_state", False))
        trace["next_pickup_stale_risk"] = bool(note_stats.get("next_pickup_stale_risk", False))
        trace["next_pickup_actionable"] = bool(note_stats.get("next_pickup_actionable", False))
        trace["session_note_compaction_used"] = bool(note_stats.get("session_note_compaction_used", False))
        trace["session_note_count_before"] = int(note_stats.get("session_note_count_before", note_stats.get("count", 0)) or 0)
        trace["session_note_count_after"] = int(note_stats.get("session_note_count_after", note_stats.get("count", 0)) or 0)
        trace["session_note_size_before"] = int(note_stats.get("session_note_size_before", note_stats.get("chars", 0)) or 0)
        trace["session_note_size_after"] = int(note_stats.get("session_note_size_after", note_stats.get("chars", 0)) or 0)
        trace["session_note_compaction_preserved_items"] = list(note_stats.get("session_note_compaction_preserved_items", []) or [])
        trace["session_note_compaction_removed_duplicates"] = int(note_stats.get("session_note_compaction_removed_duplicates", 0) or 0)
        trace["world_state_compaction_used"] = bool(note_stats.get("world_state_compaction_used", False))
        trace["next_pickup_compaction_used"] = bool(note_stats.get("next_pickup_compaction_used", False))
        _set_runtime_trace({}, context, trace)
    except Exception:
        return
...
def _raw_system_memory_reason(context, output: str, step9_mode: str) -> tuple[bool, str]:
    ...
    if any(
        ...
        return False, "memory_write_skip:raw_system_takeover_output"
    if step9_mode == "crisis_minimal" or str(getattr(context, "turn_load_level", "")).strip().lower() == "crisis":
...
def _record_duplicate_skip_if_needed(context, result: dict | None) -> None:
    if isinstance(result, dict) and result.get("reason") == "duplicate_recent":
        ...
        trace["memory_duplicate_target"] = str(result.get("memory_type", "") or "")
        trace["memory_duplicate_reason"] = str(result.get("reason", "") or "duplicate_recent")
        trace["memory_write_skipped_duplicate"] = True
        _set_runtime_trace({}, context, trace)
...
def _record_session_note_size_warnings(context) -> None:
    ...
    if note_size > 12000 or note_count > 30:
        trace["session_note_compaction_needed"] = True
        reasons = list(trace.get("session_note_compaction_reason", []) or [])
        if note_size > 12000 and "session_note_size_high" not in reasons:
            reasons.append("session_note_size_high")
        if note_count > 30 and "session_note_count_high" not in reasons:
            reasons.append("session_note_count_high")
        trace["session_note_compaction_reason"] = reasons
        _set_runtime_trace({}, context, trace)
...
def _record_focus_material_trace(context, material: dict) -> None:
    ...
    next_pickup_text = _format_material_note(material, kind="next_pickup")
    session_text = _format_material_note(material, kind="session_note")
    world_text = _format_material_note(material, kind="world_state")
    trace["next_pickup_duplicate_with_session_note"] = bool(next_pickup_text and next_pickup_text in session_text)
    trace["next_pickup_duplicate_with_world_state"] = bool(next_pickup_text and next_pickup_text in world_text)
    trace["next_pickup_actionable"] = bool(next_pickup_text and any(token in next_pickup_text for token in ("判断", "建议", "方向", "下一步", "避免")))
    if world_text:
        trace["world_state_latest_update_source"] = "step9:material_summary"
