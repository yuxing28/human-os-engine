...
class SessionMemory:
    ...
    def __init__(self, storage_dir: str = "data/sessions"):
        ...
        self._duplicate_check_window = 12
        self._last_compaction_events: dict[str, dict[str, Any]] = {}

    @staticmethod
    def _normalize_note_text(content: str) -> str:
        return " ".join((content or "").strip().lower().split())

    def _note_signature(self, note_type: str, content: str, detail: dict[str, Any] | None = None) -> str:
        detail = detail or {}
        normalized = self._normalize_note_text(content)
        if note_type == "world_state":
            return "|".join(
                [
                    note_type,
                    str(detail.get("scene_id", "") or ""),
                    str(detail.get("situation_stage", "") or ""),
                    str(detail.get("progress_state", "") or ""),
                    str(detail.get("commitment_state", "") or ""),
                    str(detail.get("next_turn_focus", "") or ""),
                ]
            )
        if note_type == "action_loop":
            return "|".join(
                [
                    note_type,
                    str(detail.get("scene_id", "") or ""),
                    ...
                    str(detail.get("commitment_state", "") or ""),
                    str(detail.get("next_turn_focus", "") or ""),
                    str(detail.get("active_goal", "") or ""),
                ]
            )
        if note_type == "relationship_state":
            return "|".join(
                [
                    note_type,
                    str(detail.get("scene_id", "") or ""),
                    str(detail.get("relationship_position", "") or ""),
                    str(detail.get("situation_stage", "") or ""),
                ]
            )
        if note_type == "state_evolution":
            return "|".join(
                [
                    note_type,
                    self._normalize_note_text(str(detail.get("from", "") or "")),
                    self._normalize_note_text(str(detail.get("to", "") or "")),
                    normalized,
                ]
            )
        ...
        return f"{note_type}|{normalized}"

    def _find_recent_duplicate_index(
        self,
        session_id: str,
        note_type: str,
        content: str,
        ...
        if not notes:
            return None
        target = self._note_signature(note_type, content, detail)
        for idx in range(len(notes) - 1, max(-1, len(notes) - self._duplicate_check_window - 1), -1):
            note = notes[idx]
            if note.note_type != note_type:
                continue
            if self._note_signature(note.note_type, note.content, note.detail) == target:
                return idx
        return None

    def _find_recent_merge_target(
        self,
        session_id: str,
        note_type: str,
        detail: dict[str, Any] | None = None,
        ...
        detail = detail or {}
        if note_type not in {"world_state", "action_loop", "relationship_state"}:
            return None
        notes = self._notes.get(session_id, [])
        if not notes:
            return None
        target_scene = str(detail.get("scene_id", "") or "")
        for idx in range(len(notes) - 1, max(-1, len(notes) - self._duplicate_check_window - 1), -1):
            note = notes[idx]
            if note.note_type != note_type:
                continue
            note_scene = str((note.detail or {}).get("scene_id", "") or "")
            if target_scene and note_scene and target_scene == note_scene:
                return idx
        return None
    ...
    def add_note(
        ...
        duplicate_idx = self._find_recent_duplicate_index(session_id, note_type, content, detail)
        if duplicate_idx is not None:
            return {
                "status": "skipped",
                "reason": "duplicate_recent_note",
                "note_type": note_type,
                "duplicate_target": note_type,
            }

        merge_idx = self._find_recent_merge_target(session_id, note_type, detail)
        if merge_idx is not None:
            existing = self._notes[session_id][merge_idx]
            self._notes[session_id][merge_idx] = SessionNote(
                round_num=round_num,
                note_type=note_type,
                content=content,
                detail=detail,
            ...
            compaction = self._compact_session_notes_if_needed(session_id)
            self._save_session(session_id)
            return {
                "status": "merged",
                "reason": "stateful_note_replaced",
                "note_type": note_type,
                "duplicate_target": note_type,
    ...
    @staticmethod
    def _note_is_preserved(content: str, detail: dict[str, Any] | None = None) -> bool:
        text = f"{content or ''} {json.dumps(detail or {}, ensure_ascii=False)}"
        preserve_markers = (
            "crisis",
            "安全",
            "自杀",
    ...
    def _compact_session_notes_if_needed(self, session_id: str) -> dict[str, Any]:
        notes = self._notes.get(session_id, [])
        count_before = len(notes)
        size_before = sum(len(note.content or "") for note in notes)
        recent = notes[-12:]
        signatures = Counter(self._note_signature(note.note_type, note.content, note.detail) for note in recent)
        duplicate_hits = sum(count - 1 for count in signatures.values() if count > 1)
        duplicate_score = round(duplicate_hits / max(1, len(recent)), 3)
        reasons: list[str] = []
        if count_before > 8:
            reasons.append("session_note_count_high")
        if size_before > 900:
            reasons.append("session_note_size_high")
        if duplicate_score > 0.75:
            reasons.append("session_note_duplicate_dense")
        if not reasons:
            previous = self._last_compaction_events.get(session_id, {})
            if previous.get("used"):
                return previous
            event = {
                "used": False,
                "reason": [],
                ...
                "world_state_compaction_used": False,
                "next_pickup_compaction_used": False,
            }
            self._last_compaction_events[session_id] = event
            return event

        keep: set[int] = set()
        preserved_items: list[str] = []
        preserved_contents: list[str] = []
        latest_by_type: dict[str, int] = {}
        stateful_types = {"world_state", "action_loop", "relationship_state", "state_evolution"}
        for index, note in enumerate(notes):
            if note.note_type == "compacted_summary":
                preserved_contents.append((note.content or "").replace("压缩保留: ", "")[:160])
                continue
            if note.note_type in stateful_types:
                latest_by_type[note.note_type] = index
            if self._note_is_preserved(note.content, note.detail):
                if len(preserved_items) < 8:
                    preserved_items.append(note.note_type)
                preserved_contents.append((note.content or "")[:120])

        keep.update(latest_by_type.values())
        keep.update(range(max(0, count_before - 5), count_before))

        seen_signatures: set[str] = set()
        compacted: list[SessionNote] = []
        removed_duplicates = 0
        for index, note in enumerate(notes):
            if note.note_type == "compacted_summary":
                continue
            signature = self._note_signature(note.note_type, note.content, note.detail)
            if signature in seen_signatures and index not in keep:
        ...
        if preserved_contents:
            unique_preserved = list(dict.fromkeys(preserved_contents))

            def _preserve_priority(item: str) -> int:
                if any(marker in item for marker in ("偏好", "先给结论", "不要总反问", "以后")):
                    return 0
                if any(marker in item for marker in ("crisis", "安全", "不想活", "恢复")):
                    return 1
                if any(marker in item for marker in ("repair", "没答到", "补答", "未完成")):
                    return 2
                if any(marker in item for marker in ("修正", "价格不动", "服务", "不能超过", "已修正")):
                    return 3
                return 9

            unique_preserved.sort(key=_preserve_priority)
            summary_text = "；".join(unique_preserved[:8])[:420]
            compacted.append(
                SessionNote(
                    round_num=max((note.round_num for note in notes), default=0),
                    note_type="compacted_summary",
                    content=f"压缩保留: {summary_text}",
                    detail={"source": "session_note_compaction", "preserved_items": preserved_items[:8]},
                )
            )

        if len(compacted) > 10:
            always_keep = [note for note in compacted if note.note_type in stateful_types or note.note_type == "compacted_summary"]
            recent_keep = compacted[-5:]
            selected: list[SessionNote] = []
            selected_ids: set[int] = set()
            for note in always_keep:
                note_id = id(note)
                if note_id in selected_ids:
                    continue
                selected.append(note)
                selected_ids.add(note_id)
            remaining_slots = max(0, 10 - len(selected))
            for note in recent_keep[-remaining_slots:] if remaining_slots else []:
                note_id = id(note)
                if note_id in selected_ids:
                    continue
                selected.append(note)
        ...
        compacted.sort(key=lambda note: (note.round_num, note.timestamp))
        self._notes[session_id] = compacted
        size_after = sum(len(note.content or "") for note in compacted)
        event = {
            "used": True,
            "reason": reasons,
            "count_before": count_before,
            "count_after": len(compacted),
            "size_before": size_before,
            "size_after": size_after,
            "preserved_items": preserved_items[:8],
            "removed_duplicates": max(removed_duplicates, count_before - len(compacted)),
            "world_state_compaction_used": any(note.note_type == "world_state" for note in notes),
            "next_pickup_compaction_used": any(
                str((note.detail or {}).get("next_turn_focus", "") or "") for note in notes
            ),
        }
        self._last_compaction_events[session_id] = event
        return event
    ...
    def get_note_stats(self, session_id: str) -> dict[str, int]:
        ...
        signatures: Counter[str] = Counter(
            self._note_signature(note.note_type, note.content, note.detail) for note in recent
        )
        duplicate_hits = sum(count - 1 for count in signatures.values() if count > 1)
        duplicate_score = round(duplicate_hits / max(1, len(recent)), 3)
        type_counts = Counter(note.note_type for note in notes)
        repeated_entities = [name for name, count in type_counts.items() if count > 1][:5]

        world_state_notes = [note for note in notes if note.note_type == "world_state"]
        world_state_signatures = Counter(
            self._note_signature(note.note_type, note.content, note.detail) for note in world_state_notes[-6:]
        )
        world_state_duplicate_score = round(
            sum(count - 1 for count in world_state_signatures.values() if count > 1) / max(1, len(world_state_notes[-6:] or [])),
            3,
        ) if world_state_notes else 0.0
        latest_world_state = world_state_notes[-1] if world_state_notes else None
        latest_world_detail = latest_world_state.detail if latest_world_state else {}
        latest_focus = str(latest_world_detail.get("next_turn_focus", "") or "")
        latest_focus_norm = self._normalize_note_text(latest_focus)

        closure_notes = [note for note in notes if note.note_type == "closure"]
        recent_closure_text = " ".join((note.content or "") for note in closure_notes[-2:])
        next_pickup_duplicate_with_session_note = bool(latest_focus_norm and latest_focus_norm in self._normalize_note_text(recent_closure_text))
        next_pickup_duplicate_with_world_state = bool(latest_focus)
        next_pickup_actionable = bool(latest_focus and any(marker in latest_focus for marker in ("先", "再", "下一步", "确认", "跟进", "收", "发", "问", "守")))

        repeated_intents = []
        seen_focus = set()
        for note in notes:
            focus = str((note.detail or {}).get("next_turn_focus", "") or "")
            if not focus:
                continue
            normalized_focus = self._normalize_note_text(focus)
            if normalized_focus in seen_focus:
                repeated_intents.append(focus[:24])
            else:
                seen_focus.add(normalized_focus)
        repeated_intents = repeated_intents[:5]

        compaction_needed = chars > 12000 or len(notes) > 30 or duplicate_score >= 0.3
        compaction_reason = []
        if chars > 12000:
            compaction_reason.append("session_note_size_high")
        if len(notes) > 30:
            compaction_reason.append("session_note_count_high")
        if duplicate_score >= 0.3:
            compaction_reason.append("session_note_duplicate_dense")
        compaction_event = self._last_compaction_events.get(session_id, {})
        ...
        return {
            ...
            "repeated_intents": repeated_intents,
            "compaction_needed": compaction_needed,
            "compaction_reason": compaction_reason,
            "world_state_duplicate_score": world_state_duplicate_score,
            "world_state_stale_risk": len(world_state_notes) > 3 and world_state_duplicate_score > 0.2,
            "world_state_conflict_detected": False,
            "world_state_latest_update_source": latest_world_state.note_type if latest_world_state else "",
            "next_pickup_duplicate_with_session_note": next_pickup_duplicate_with_session_note,
            "next_pickup_duplicate_with_world_state": next_pickup_duplicate_with_world_state,
            "next_pickup_stale_risk": False,
            "next_pickup_actionable": next_pickup_actionable,
            "session_note_compaction_used": bool(compaction_event.get("used", False)),
            "session_note_count_before": int(compaction_event.get("count_before", len(notes)) or 0),
            "session_note_count_after": int(compaction_event.get("count_after", len(notes)) or 0),
            "session_note_size_before": int(compaction_event.get("size_before", chars) or 0),
            "session_note_size_after": int(compaction_event.get("size_after", chars) or 0),
            "session_note_compaction_preserved_items": list(compaction_event.get("preserved_items", []) or []),
            "session_note_compaction_removed_duplicates": int(compaction_event.get("removed_duplicates", 0) or 0),
            "world_state_compaction_used": bool(compaction_event.get("world_state_compaction_used", False)),
            "next_pickup_compaction_used": bool(compaction_event.get("next_pickup_compaction_used", False)),
        }
    ...
    def get_context_for_llm(self, session_id: str, limit: int = 5) -> str:
        ...
        summarized_types = {"world_state", "state_evolution", "relationship_state", "action_loop", "closure"}
        recent_free_notes: list[SessionNote] = []
        seen_note_signatures: set[str] = set()
        for note in reversed(notes):
            if note.note_type in summarized_types:
                continue
            signature = self._note_signature(note.note_type, note.content, note.detail)
            if signature in seen_note_signatures:
                continue
            seen_note_signatures.add(signature)
            recent_free_notes.append(note)
            if len(recent_free_notes) >= 3:
                break
        for note in reversed(recent_free_notes):
            lines.append(f"- 第{note.round_num}轮 [{note.note_type}]: {note.content}")
...
def add_session_note(
    ...
    """添加会话笔记"""
    return get_session_memory().add_note(session_id, round_num, note_type, content, detail)
